...
class DocumentProgressTracker:
    """Redis-based document processing progress tracker."""

    PROGRESS_KEY_PREFIX = "doc_progress:"
    PROGRESS_TTL = 3600  # 1 hour TTL
    _redis = None

    @classmethod
    def _get_redis(cls):
        """Get direct Redis connection (not Flask-dependent)."""
    ...
    @staticmethod
    def _get_key(document_id: str) -> str:
        return f"{DocumentProgressTracker.PROGRESS_KEY_PREFIX}{document_id}"

    @classmethod
    def set_progress(cls, document_id: str, stage: str, progress: int,
                     message: str = "", total_pages: int = 0, current_page: int = 0):
        ...
        try:
            ...
            cls._get_redis().setex(
                cls._get_key(document_id),
                cls.PROGRESS_TTL,
                json.dumps(data)
            )
        except Exception as e:
            logging.warning(f"Failed to set progress for {document_id}: {e}")

    @classmethod
    def get_progress(cls, document_id: str) -> Optional[dict]:
        """Get document processing progress from Redis."""
        try:
            data = cls._get_redis().get(cls._get_key(document_id))
            if data:
                return json.loads(data)
        except Exception as e:
            logging.warning(f"Failed to get progress for {document_id}: {e}")
        return None

    @classmethod
    def clear_progress(cls, document_id: str):
        """Clear document processing progress from Redis."""
        try:
            cls._get_redis().delete(cls._get_key(document_id))
        except Exception as e:
            logging.warning(f"Failed to clear progress for {document_id}: {e}")
    ...
    def run(self, dataset_documents: list[DatasetDocument]):
        ...
        for dataset_document in dataset_documents:
            try:
                ...
                DocumentProgressTracker.set_progress(
                    dataset_document.id,
                    "parsing",
                    0,
                ...
                DocumentProgressTracker.set_progress(
                    ...
                    "splitting",
                    60,
                    f"Saving {len(documents)} segments..."
                )
                ...
                DocumentProgressTracker.clear_progress(dataset_document.id)
            except Exception as e:
        ...
        if dataset_document.data_source_type == "upload_file":
            ...
            if file_detail:
                ...
                DocumentProgressTracker.set_progress(
                    ...
                    "parsing",
                    35,
                    f"Extracted {len(text_docs)} text blocks"
                )
        ...
        if dataset.indexing_technique == "high_quality":
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                ...
                for idx, future in enumerate(futures):
                    ...
                    progress = 70 + int((idx + 1) / len(futures) * 25)
                    DocumentProgressTracker.set_progress(
                        dataset_document.id,
                        "indexing",
                        progress,
                        f"Indexing chunk {idx + 1}/{len(futures)}..."
                    )
